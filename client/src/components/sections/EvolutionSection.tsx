          ...
          {phases.map((p, i) => (
            <AnimatedSection key={p.phase} delay={i * 0.15}>
              <GlowCard glowColor={p.color} className="p-6 h-full">
                ...
                <p className="text-xs text-muted-foreground/75 mb-3">{p.title}</p>
                <p className="text-sm text-muted-foreground leading-relaxed">{p.descEn}</p>
                <p className="text-xs text-muted-foreground/65 leading-relaxed mt-1">{p.desc}</p>
              </GlowCard>
            </AnimatedSection>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {metrics.map((m, i) => (
              <div key={i} className="text-center">
                ...
                <p className="text-xs text-muted-foreground/65">{m.label}</p>
              </div>
            ))}
          </div>
        </AnimatedSection>
